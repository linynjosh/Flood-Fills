                                         ...
                                         int centerX, int centerY)
{
     ...
     */
    fade_color1 = fadeColor1;
    fade_color2 = fadeColor2;
    rad = radius;
    center_x = centerX;
    center_y = centerY;
}
...
HSLAPixel gradientColorPicker::operator()(int x, int y)
{
    ...
    double dist = abs(x - center_x) + abs(y - center_y);
    double longest_dist = fade_color1.h - fade_color2.h;
    color.s = 1.;
    color.l = 0.5;
    if (dist > rad) {
        return fade_color2;
    }
    if (longest_dist && abs(longest_dist) >= 180) {
        color.h = fade_color1.h + (dist/rad) * (abs(longest_dist));
    } else if (!longest_dist && abs(longest_dist) >= 180) {
        color.h = fade_color1.h + (dist/rad) * (abs(longest_dist));
    } else if (longest_dist && abs(longest_dist) < 180) {
        color.h = fade_color1.h + (dist/rad) * (360 - abs(longest_dist));
    } else if (!longest_dist && abs(longest_dist) < 180) {
        color.h = fade_color1.h + (dist/rad) * (360 - abs(longest_dist));
    }
    if (color.h > 360) {
